...
class OperacoesMatematicas:
    ...
    def soma(self):
        ...
        """
        return self.aplicar_operacao(lambda img1, img2: np.clip(img1 + img2, 0, 255))
    ...
    def divisao(self):
        ...
        """
        return self.aplicar_operacao(lambda img1, img2: np.clip(
            np.divide(img1, img2, out=np.zeros_like(img1, dtype=np.float32), where=img2 != 0), 0, 255
        ).astype(np.uint8))
